...
description: "Execute TradingView PineScript strategies on Hyperliquid using Hyperscript's webhook integration and automated execution engine"
---

## What is PineScript Integration?

Hyperscript's PineScript Integration allows you to execute your TradingView Pine Script strategies directly on Hyperliquid through webhook automation. This bridges the gap between TradingView's powerful charting and strategy development environment and real-world trading execution.

## How PineScript Integration Works

Connect your TradingView strategies to Hyperscript for seamless automated execution:

<Info>
  **Example Workflow**: 1. Develop your strategy in TradingView Pine Script 2.
  Configure webhook alerts in TradingView 3. Connect webhook to Hyperscript
  execution engine 4. Hyperscript automatically executes trades on Hyperliquid
...
graph TD
    ...
    C --> D[Hyperscript Webhook Receiver]
...
```pinescript
//@version=5
strategy("Hyperscript Integration Example", overlay=true, default_qty_type=strategy.percent_of_equity, default_qty_value=10)
...
3. **Configure Webhook**:
   - **Webhook URL**: `https://api.hyperscript.xyz/webhook/tv/{your-webhook-id}`
...
### Step 2: Hyperscript Configuration

Connect your TradingView alerts to Hyperscript execution:
...
Hyperscript supports flexible JSON message formats:
...
```pinescript
//@version=5
strategy("Hyperscript EMA Cross", overlay=true,
...
```pinescript
//@version=5
strategy("Hyperscript RSI Reversal", overlay=false,
...
**Problem**: Alerts not reaching Hyperscript
...
>
  Combine PineScript with Hyperscript's proprietary signals.
</Card>
...
<Note>
  ...
  while maintaining the safety and risk management features of the Hyperscript
